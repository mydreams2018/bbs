...
<head>
    ...
    <script type="text/javascript">
        ...
        function editPosts(obj){
            window.location.replace("/dataDetails/update?id="+obj);
        };

        function pageChange(obj) {
            var pag = $("#"+obj).attr("data-crt");
            window.location.replace("/dataPosts/details?id="+postsId+"&currentPage="+pag);
        };
        ...
        function deleteDeatils(obj) {
            $.ajax({
                url: "/manager/deleteData",
                type: "post",
                data: {
                    "id":obj
                },
...
<div class="row">
    <div class="col-md">
        <h1 class="text-center" style="color: #ff5479">${(posts.postsName)!'无名'}</h1>
    </div>
</div>
                    ...
                    <div class="card col-md-9">
                        ...
                        <div class="card-footer text-right">
                            <small class="text-muted">
                                ...
                                <a href="javascript:deleteDeatils(${(data.id)!})" class="card-link">删除</a>
...
<#if (Session.KUN_CURRENT_NAME)?? >
    <div class="row pading30 justify-content-end">
        <div class="col-md-9">
            <form id="send-data-posts" method="post" action="/dataDetails/save">
