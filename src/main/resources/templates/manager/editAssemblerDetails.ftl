...
<head>
    ...
    <script type="text/javascript">
        ...
        function editPosts(obj){
            window.location.replace("/assemblerDetails/update?id="+obj);
        };

        function pageChange(obj) {
            var pag = $("#"+obj).attr("data-crt");
            window.location.replace("/assemblerPosts/details?id="+postsId+"&currentPage="+pag);
        };
        ...
        function sustain(obj) {
            ...
            $.ajax({
                url: "/assemblerDetailsRecord/save",
                type: "post",
                data: {
                    "state":1,
                    "assemblerDetailsId":obj
                },
                dataType: "json",
                success: function (data) {
                    if(data.result){
...
<#if (Session.KUN_CURRENT_NAME)?? >
    <div class="row pading30 justify-content-end">
        <div class="col-md-9">
            <form id="send-java-posts" method="post" action="/assemblerDetails/save">
